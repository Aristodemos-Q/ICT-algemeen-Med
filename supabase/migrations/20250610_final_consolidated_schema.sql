-- =======================================
-- BV FLORIANDE FINAL CONSOLIDATED SCHEMA
-- =======================================
-- Complete database schema for BV Floriande Trainers Platform
-- Consolidated from ALL working migrations, NO SAMPLE DATA included
-- Date: 2025-06-10
-- Version: Final Clean
...
CREATE TABLE public.members (
    ...
    name TEXT NOT NULL,
    birth_date DATE,
    date_of_birth DATE, -- Alias for birth_date for backward compatibility
    contact_info TEXT,
    email TEXT,
    phone TEXT,
...
-- Enhanced admin check function (from 20250606_add_admin_function.sql)
...
-- Safe RLS policies (from 20250531_fix_infinite_recursion_rls_v2.sql)
-- Users policies - avoid recursion by checking auth.users directly
...
-- Other tables - use simple public access for authenticated users
...
CREATE POLICY "Authenticated users can manage group members" ON public.group_members 
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can view group trainers" ON public.group_trainers 
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can manage group trainers" ON public.group_trainers 
  FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Authenticated users can view session trainers" ON public.session_trainers 
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can manage session trainers" ON public.session_trainers 
  FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Authenticated users can view completed exercises" ON public.completed_exercises 
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can manage completed exercises" ON public.completed_exercises 
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can view group evaluations" ON public.group_evaluations 
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can manage group evaluations" ON public.group_evaluations 
  FOR ALL USING (auth.role() = 'authenticated');
...
GRANT EXECUTE ON FUNCTION public.check_admin_role() TO authenticated, anon, service_role;
...
DO $$
BEGIN
  RAISE NOTICE '=== BV FLORIANDE FINAL SCHEMA SETUP COMPLETE ===';
  RAISE NOTICE 'Database schema has been successfully created and consolidated';
  RAISE NOTICE 'Features included:';
  RAISE NOTICE '  ✅ Complete table structure with all latest fields';
  RAISE NOTICE '  ✅ Safe RLS policies (no infinite recursion)';
  RAISE NOTICE '  ✅ Admin functions and health checks';
  RAISE NOTICE '  ✅ Performance indexes';
  RAISE NOTICE '  ✅ Updated triggers and constraints';
  RAISE NOTICE '  ❌ NO SAMPLE DATA - all data to be added via web interface';
